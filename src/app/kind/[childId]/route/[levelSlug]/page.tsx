...
import { getCategoriesForLevel, getLevelBySlug, type CategoryDefinition } from "@/lib/contentCatalog";
...
export default function LevelDetailPage() {
  ...
  const [allCategories, setAllCategories] = useState<CategoryDefinition[] | null>(null);

  useEffect(() => {
    let cancelled = false;
    getWordsContent().then((content) => {
      if (!cancelled) setAllCategories(mergeCategories(content));
    });
    return () => {
      cancelled = true;
    };
  ...
  if (!ready || allCategories === null) return <p className="pt-12 text-center text-ink-muted">Even laden…</p>;
  ...
  const categories = getCategoriesForLevel(level.slug, allCategories);
